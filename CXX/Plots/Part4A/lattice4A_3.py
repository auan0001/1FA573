...
plt.axis("off")
lat1 = np.loadtxt(path + '/S_32_0_3' + '.dat', delimiter=' ').astype(int).reshape(N,N)
lat2 = np.loadtxt(path + '/S_32_37_3' + '.dat', delimiter=' ').astype(int).reshape(N,N)
lat3 = np.loadtxt(path + '/S_32_50_3' + '.dat', delimiter=' ').astype(int).reshape(N,N)
lat4 = np.loadtxt(path + '/S_32_64_3' + '.dat', delimiter=' ').astype(int).reshape(N,N)
...
fig.suptitle("$L=32$ lattice with $J=-1$, $B=3$", fontsize='16')
