...
for i in range(start,stop+1):
    sym = col['order']
    x = np.genfromtxt(path1 + '/ising_' + str(2**i) + '_' + str(part1) + '.dat',
                      skip_header=1,
                      skip_footer=0,
                      names=True,
                      dtype=float,
    ...
    y = np.genfromtxt(path1 + '/ising_' + str(2**i) + '_' + str(part1) + '.dat',
                       skip_header=1,
                       skip_footer=0,
                       names=True,
                       dtype=float,
...
for i in range(start,stop+1):
    ...
    y = np.genfromtxt(path1 + '/ising_' + str(2**i) + '_' + str(part1) + '.dat',
                       ...
                       delimiter=' ',
                       usecols=(sym,)).astype(float)
    ax[1].plot(x, y*(2**i), label = r'$B='+str(i)+'$', marker='.', linestyle='--', linewidth='1', markersize='3')
ax[1].grid()
...
for i in range(start,stop+1):
    ...
    y = np.genfromtxt(path1 + '/ising_' + str(2**i) + '_' + str(part1) + '.dat',
                       ...
                       delimiter=' ',
                       usecols=(sym,)).astype(float)
    ax[2].plot(x, y/(2**i), label = r'$L='+str(2**i)+'$', marker='.', linestyle='--', linewidth='1', markersize='3')
ax[2].grid()
ax[2].set_title(r'Specific heat', fontsize = '18')
ax[2].set_xlabel(r'$k_BT$', fontsize = '18')
ax[2].set_ylabel(r'$C_B$', fontsize = '18')

fig.suptitle(r'Varying $L$, $J=1$, $B=0$ over $200000$ sweeps', fontsize='22')
